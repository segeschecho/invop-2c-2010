...
Puede pasar que haciendo bfs, no se recorra todo el grafo, ya que el mismo puede no ser conexo, por esta razón lo que se hace es seguir con el proceso de bfs y de coloreo para algún nodo que haya quedado sin marcar, hasta que todo el grafo haya sido recorrido.

Hasta la ronda número 4, se utilizó $heuristicBFSUpperBound$ como se detalla en el párrafo anterior. Para las pruebas que siguen, se optimizó el método, haciendo que los nodos del grafo se encuentren ordenados por su grado, y de esta forma comenzar el proceso de bfs por el nodo de mayor grado. Otra mejora que se le hizo al método, es asignarle un parámetro adicional, que indica cuantas veces se hace $heuristicBFSUpperBound$ para el grafo, que aprovecha el ordenamiento de mayor a menor grado de los nodos, y en cada corrida, va tomando como nodo inicial, el nodo de mayor grado disponible en ese momento (que no haya sido usado para corridas anteriores).
